...
from app.core.token_store import MemoryTokenStore, get_token_store, reset_token_store
...
def test_blacklist_ttl_matches_remaining_access_token_lifetime(
    ...
    assert store.is_access_blacklisted(jti)

    with store._lock:
        blacklist_exp = store._blacklist[jti]
    # Allow small clock skew; TTL must be ~ remaining JWT life, not a tiny constant.
    ttl_left = blacklist_exp - datetime.now(UTC).timestamp()
...
def test_refresh_rotation_grace_allows_parallel_duplicate_refresh(
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        codes = [f.result() for f in as_completed([pool.submit(_refresh) for _ in range(2)])]
...
def test_refresh_after_grace_expires_is_rejected(client: TestClient) -> None:
    ...
    with store._lock:
        if jti in store._grace:
            store._grace[jti] = (store._grace[jti][0], 0.0)
